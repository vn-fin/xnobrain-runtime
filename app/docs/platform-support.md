...
| Debian/Ubuntu x86-64 | Docker's official Apt repository installs Engine, CLI, Buildx, and Compose plugin | single `.AppImage` | Ubuntu/Debian native clean-install test |
| Fedora/RHEL/CentOS x86-64 | Docker's official RPM repository installs Engine, CLI, Buildx, and Compose plugin | single `.AppImage` | Declared supported distro matrix; native clean-install test |
...
The Docker-first Linux download is one AppImage under `binary/`; DEB/RPM are
optional future repository-integration formats. Release signing remains a
separate gate. Docker license acceptance always remains an explicit user
action.
