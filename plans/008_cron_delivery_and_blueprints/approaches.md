...
### A2. XNOBrain post-run delivery on the Kanban-backed job

Keep the Kanban dispatcher as the only scheduler. After an occurrence completes,
a XNOBrain service step reads the safe output and routes it to each attached
target, reusing Hermes's `DeliveryRouter` for channel/email and XNOBrain's own
...
- Cons: XNOBrain owns the delivery orchestration (dedup key, result recording),
  duplicating a slice of what the gateway ledger does for messaging.

### A3. Hybrid — native routing for messaging, XNOBrain for the rest
...
transport; do kanban/file in XNOBrain.
...
- Cons: two delivery code paths (router-backed vs XNOBrain-backed) to test.
...
### B2. XNOBrain-authored template store
...
skills}` spec, then feed it into XNOBrain's existing Kanban schedule create
...
- Cons: the blueprint's `deliver` value must be translated into an attached
  XNOBrain delivery target rather than a native `deliver` string.
...
3. **All four target types work.** Native delivery covers only messaging +
   ...
   `file` (via the workspace/files repo) as XNOBrain-owned deliveries, which the
...
5. **Safety and idempotency stay ours.** A XNOBrain per-(occurrence, target)
...
Cost accepted: two delivery code paths (router-backed messaging vs XNOBrain
kanban/file) and XNOBrain-owned orchestration. Both are small, explicitly
