...
- [findings.md](findings.md) — 9router's native multi-account model and
  XNOBrain's current single-connection collapse.
...
(program principles),
[`xnobrain/routes/setup.py`](../../xnobrain/routes/setup.py) (the only
...
Round-robin between accounts per provider", per its README) in XNOBrain. The
...
XNOBrain currently collapses that to one connection per provider and hides
...
nothing in XNOBrain** — no new files under `DATA_DIR`, so snapshot rules do
not apply — and XNOBrain continues to never see, store, return, or log API
...
- **No load-balancing policy engine in XNOBrain.** Rotation/fallback among
...
- [`xnobrain/routes/setup.py`](../../xnobrain/routes/setup.py) is the only
  route-assembly point. Handlers own HTTP, services own rules, integrations
  adapt 9router over HTTP, models are Pydantic.
- **Credentials never enter XNOBrain.** Keys live only in 9router; every
