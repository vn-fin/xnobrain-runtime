...
class _TeamRunBase(unittest.IsolatedAsyncioTestCase):
    ...
    async def _poll_until_terminal(self, client, team_id, run_id, tries=100):
        for _ in range(tries):
            response = await client.get(f"/xnobrain/api/runtime/v1/teams/{team_id}/runs/{run_id}")
...
class TeamRunLifecycleTests(_TeamRunBase):
    ...
    async def test_completed_run_can_be_deleted(self):
        async with self.client() as client:
            ...
            deleted = await client.delete(f"/xnobrain/api/runtime/v1/teams/{team_id}/runs/{run_id}")
            missing = await client.get(f"/xnobrain/api/runtime/v1/teams/{team_id}/runs/{run_id}")
