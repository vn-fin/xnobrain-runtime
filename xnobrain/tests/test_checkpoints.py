...
class CheckpointAPITests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.composition = XNOBrainApplication(
            AgentManager(root_profile=self.root, profiles_root=self.profiles, legacy_agents_root=base / "legacy"),
    ...
    async def test_write_list_diff_file_versions_and_restore(self):
        async with self.client() as client:
            ...
            disabled = await client.get(f"/xnobrain/api/runtime/v1/agents/{agent}/checkpoints/status")
            self.assertFalse(disabled.json()["data"]["enabled"])
            await client.patch(
                f"/xnobrain/api/runtime/v1/agents-configs/{agent}",
                json={"checkpoints_enabled": True},
            )
            await client.post(f"/xnobrain/api/runtime/v1/agents-workspaces/{agent}/write", json={"path": "notes.txt", "content": "one\n"})
            await client.post(f"/xnobrain/api/runtime/v1/agents-workspaces/{agent}/write", json={"path": "notes.txt", "content": "two\n"})
            listed = await client.get(f"/xnobrain/api/runtime/v1/agents/{agent}/checkpoints")
            self.assertEqual(listed.status_code, 200, listed.text)
            items = listed.json()["data"]["items"]
            self.assertGreaterEqual(len(items), 2)
            target = items[0]["id"]
            diff = await client.get(f"/xnobrain/api/runtime/v1/agents/{agent}/checkpoints/{target}/diff")
    ...
    async def test_rename_rejects_overwrite_and_is_checkpointed(self):
        async with self.client() as client:
            ...
            await client.post(f"/xnobrain/api/runtime/v1/agents-workspaces/{agent}/write", json={"path": "old.txt", "content": "old"})
            renamed = await client.post(f"/xnobrain/api/runtime/v1/agents-workspaces/{agent}/rename", json={"path": "old.txt", "new_name": "new.txt"})
            await client.post(f"/xnobrain/api/runtime/v1/agents-workspaces/{agent}/write", json={"path": "taken.txt", "content": "x"})
            conflict = await client.post(f"/xnobrain/api/runtime/v1/agents-workspaces/{agent}/rename", json={"path": "new.txt", "new_name": "taken.txt"})
    ...
    async def test_checkpoint_hashes_are_confined_to_the_owning_workspace(self):
        async with self.client() as client:
            ...
            malformed = await client.get(f"/xnobrain/api/runtime/v1/agents/{first}/checkpoints/not-a-hash/diff")
