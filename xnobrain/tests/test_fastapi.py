...
class StudioFastAPITests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_local_only_deployment_has_no_enterprise_proxy_routes(self):
        with patch.dict(os.environ, {"ENTERPRISE_API_URL": "https://control.example.test"}):
            async with self.client() as client:
                deployment = await client.get("/xnobrain/api/runtime/v1/system/deployment")
                dashboard = await client.get("/xnobrain/api/runtime/v1/dashboard/overview")
                skills = await client.get("/xnobrain/api/runtime/v1/skills")
    ...
    async def test_sandbox_detail_reports_only_important_runtime_usage(self):
        async with self.client() as client:
            response = await client.get("/xnobrain/api/runtime/v1/sandboxes/detail")
        self.assertEqual(response.status_code, 200, response.text)
    ...
    async def test_profile_registry_uses_generated_ids_and_display_names(self):
        ...
        async with self.client() as client:
            updated = await client.patch(
                f"/xnobrain/api/runtime/v1/agents/{profile['id']}/metadata",
    ...
    async def test_write_approvals_default_off_and_allow_always_disables_the_selected_gate(self):
        ...
        agent_id = agent["id"]
        self.assertEqual(agent["config"]["approval_mode"], "off")
        self.assertFalse(agent["config"]["skills_write_approval"])
        self.assertFalse(agent["config"]["memory_write_approval"])
        ...
        async with self.client() as client:
            allowed = await client.post(
                "/xnobrain/api/runtime/v1/conversations/conversation/runs/run_test/approval",
    ...
    async def test_default_and_agent_skill_catalogs_stay_profile_scoped(self):
        ...
        async with self.client() as client:
            created = await client.post("/xnobrain/api/runtime/v1/agents", json={"name": "Scoped"})
        agent_id = created.json()["data"]["id"]
        ...
        async with self.client() as client:
            ...
            with patch.object(
                ...
                response = await client.get(
                    f"/xnobrain/api/runtime/v1/agents-workspaces/{agent_id}/preview",
    ...
    async def test_workspace_workbook_returns_normalized_xlsx(self):
        async with self.client() as client:
            ...
            with patch.object(
                ...
                response = await client.get(
                    f"/xnobrain/api/runtime/v1/agents-workspaces/{agent_id}/workbook",
                    params={"path": "report.xlsx"},
                )
    ...
    async def test_workspace_file_streams_content_beyond_legacy_read_limit(self):
        ...
        async with self.client() as client:
            ...
            legacy = await client.post(
                f"/xnobrain/api/runtime/v1/agents-workspaces/{agent_id}/read",
                json={"path": "large.txt"},
            )
            streamed = await client.get(
                f"/xnobrain/api/runtime/v1/agents-workspaces/{agent_id}/file",
    ...
    async def test_bundle_round_trip_exports_every_profile_file_and_redacts_credentials(self):
        ...
        async with self.client() as client:
            inspected = await client.post("/xnobrain/api/runtime/v1/bundles/inspect", files=upload)
            preview = await client.post("/xnobrain/api/runtime/v1/bundles/dry-run", files=upload)
            applied = await client.post("/xnobrain/api/runtime/v1/bundles/apply", files=upload)
    ...
    async def test_chunk_merge_rejects_non_hermes_profile_before_create(self):
        ...
        async with self.client() as client:
            ...
            completed = await client.post(
                f"/xnobrain/api/runtime/v1/bundles/uploads/{upload['upload_id']}/complete",
                json={},
            )
    ...
    async def test_team_snapshot_exports_profiles_and_remaps_the_complete_workflow(self):
        ...
        async with self.client() as client:
            ...
            imported = await client.get(f"/xnobrain/api/runtime/v1/teams/{imported_team_id}")
    ...
    async def test_chat_stream_rejects_a_conversation_that_is_not_opened(self):
        ...
        async with self.client() as client:
            ...
            streamed = await client.post(
                f"/xnobrain/api/runtime/v1/conversations/{missing_id}/chat/stream?agent={agent_id}",
    ...
    async def test_conversation_usage_is_aggregated_from_the_hermes_session(self):
        async with self.client() as client:
            ...
            response = await client.get(
                f"/xnobrain/api/runtime/v1/conversations/{conversation_id}/usage?agent={agent_id}",
            )
