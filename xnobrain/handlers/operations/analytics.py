...
def operations(handler: Any, request: Any, body: dict[str, Any]) -> dict[str, Operation]:
    ...
    return {
        ...
        "analytics_usage": (
            lambda: s.analytics.usage_summary(
                agent_ids=csv(q.get("agents")), **time_range(q), bucket=bucket(q)
            ),
            "usage analytics retrieved successfully",
            200,
        ),
        "analytics_overview": (
            lambda: s.analytics.usage_overview(
                agent_ids=csv(q.get("agents")), **time_range(q), bucket=bucket(q)
            ),
            "usage overview retrieved successfully",
            200,
        ),
        "analytics_agent_usage": (
            lambda: s.analytics.agent_usage(p["agent_id"], **time_range(q), bucket=bucket(q)),
            "agent usage retrieved successfully",
            200,
        ),
        ...
        "analytics_models": (
            lambda: s.analytics.models_breakdown(
                agent_ids=csv(q.get("agents")), **time_range(q), bucket=bucket(q)
            ),
            "model usage retrieved successfully",
            200,
        ),
        "analytics_timeseries": (
            lambda: s.analytics.timeseries(
                agent_ids=csv(q.get("agents")), **time_range(q), bucket=bucket(q)
            ),
            "usage timeseries retrieved successfully",
            200,
        ),
        "analytics_budget_get": (
            lambda: s.analytics.get_budget(p["agent_id"]),
            "budget retrieved successfully",
            200,
        ),
        "analytics_budget_set": (
            lambda: s.analytics.set_budget(p["agent_id"], body),
