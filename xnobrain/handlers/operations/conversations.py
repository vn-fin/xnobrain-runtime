...
def operations(handler: Any, request: Any, body: dict[str, Any]) -> dict[str, Operation]:
    ...
    return {
        ...
        "conversation_goal_get": (lambda: s.get_conversation_goal(agent(), p["conversation_id"]), "goal retrieved", 200),
        "conversation_goal_create": (lambda: s.create_conversation_goal(agent(), p["conversation_id"], body), "goal started", 202),
        "conversation_goal_update": (lambda: s.update_conversation_goal(agent(), p["conversation_id"], body), "goal updated", 200),
        "conversation_goal_delete": (lambda: s.delete_conversation_goal(agent(), p["conversation_id"]), "goal deleted", 200),
        "conversation_goal_pause": (lambda: s.pause_conversation_goal(agent(), p["conversation_id"]), "goal paused", 200),
        "conversation_goal_resume": (lambda: s.resume_conversation_goal(agent(), p["conversation_id"]), "goal resumed", 202),
        "conversation_subgoal_create": (lambda: s.add_conversation_subgoal(agent(), p["conversation_id"], body), "subgoal added", 201),
        "conversation_subgoal_delete": (lambda: s.remove_conversation_subgoal(agent(), p["conversation_id"], p["subgoal_index"]), "subgoal deleted", 200),
