...
def operations(handler: Any, request: Any, body: dict[str, Any]) -> dict[str, Operation]:
    ...
    return {
        ...
        "teams_update": (
            lambda: s.update_team(p["team_id"], body),
            "team updated successfully",
            200,
        ),
        "teams_delete": (lambda: s.delete_team(p["team_id"]), "team deleted successfully", 200),
        "teams_run": (
            lambda: s.run_team(p["team_id"], body),
            "team run completed successfully",
            200,
        ),
        "team_runs_start": (
            lambda: s.team_runs.start_run(p["team_id"], body),
            "team run started",
            202,
        ),
        "team_runs_list": (
            lambda: s.team_runs.list_runs(p["team_id"], q.get("limit")),
            "team runs retrieved successfully",
            200,
        ),
        "team_runs_get": (
            lambda: s.team_runs.get_run(p["team_id"], p["run_id"]),
            "team run retrieved successfully",
            200,
        ),
        "team_runs_delete": (
            lambda: s.team_runs.delete_run(p["team_id"], p["run_id"]),
            "team run deleted",
            200,
        ),
        "team_runs_cancel": (
            lambda: s.team_runs.cancel_run(p["team_id"], p["run_id"]),
            "team run cancelled",
            200,
        ),
    }
