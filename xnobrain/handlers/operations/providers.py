...
def operations(handler: Any, request: Any, body: dict[str, Any]) -> dict[str, Operation]:
    ...
    return {
        ...
        "provider_models": (
            lambda: s.provider_models(p["provider_id"]),
            "models retrieved successfully",
            200,
        ),
        "provider_reasoning": (
            lambda: s.provider_model_reasoning(p["provider_id"], p["model"]),
            "reasoning options retrieved",
            200,
        ),
        ...
        "blends_available_models": (
            s.blends.available_models,
            "blend-eligible models retrieved successfully",
            200,
        ),
        "blends_patch": (
            lambda: s.blends.update_blend(p["blend_id"], body),
            "blend updated successfully",
            200,
        ),
        "blends_delete": (
            lambda: s.blends.delete_blend(p["blend_id"]),
            "blend deleted successfully",
            200,
        ),
    }
