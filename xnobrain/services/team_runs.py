...
class TeamRunService:
    ...
    async def _execute_workflow(self, record: dict[str, Any], team: Mapping[str, Any], workflow: list[Mapping[str, Any]]) -> str:
        ...
        if coordinator_prompt:
            ...
            if coordinated.get("conversation_id"):
                record["coordinator_conversation_id"] = str(coordinated["conversation_id"])
                await self._persist(record)
        ...
        try:
            ...
            if final.get("conversation_id"):
                record["synthesis_conversation_id"] = str(final["conversation_id"])
        except asyncio.CancelledError:
            raise
        except EXPECTED_ERRORS as error:
