...
class AgentProfilesMixin:
    ...
    def _workspace_path(self, name: str, raw_path: Any, *, require_file: bool) -> Path:
        ...
        if path != root and root not in path.parents:
            raise AgentAPIError(
                "path must not escape the agent workspace", code="invalid_workspace_path"
            )
    ...
    def _write_profile_manifest(
        ...
        payload = {
            ...
            "description": str(
                metadata.get("description") or "Office and knowledge-work assistant"
            ),
        }
    ...
    def _effective_config(
        self, config: Mapping[str, Any], metadata: Mapping[str, Any]
    ) -> dict[str, Any]:
        effort = str(
        ...
        return {
            ...
            "assignment_id": str(self._get_nested(config, ("model", "assignment_id"), "") or ""),
