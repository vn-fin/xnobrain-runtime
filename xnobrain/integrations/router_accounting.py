...
def weekly_budget_decision(payload: dict[str, Any], *, application: str, environment: str,
                           workspace_id: str, agent_id: str, weekly_usd: float,
                           now: datetime | None = None) -> dict[str, Any]:
    now = now or datetime.now(timezone.utc)
    try:
        if any(payload.get(key) != expected for key, expected in {
            "capability_version": CAPABILITY, "application": application,
            "environment": environment, "workspace_id": workspace_id,
            "agent_ids": [agent_id], "timezone": "UTC", "accounting_state": "settled",
            "completeness": "durable_records", "freshness": "settled_only",
        }.items()):
            raise ValueError
    ...
    return {
        "source": "gorouter", "capability_version": CAPABILITY, "cost_basis": "accounted",
        "enforcement": "soft_admission", "currency": "USD", "weekly_usd": weekly_usd,
        "spend_usd": cost, "remaining_usd": max(0, weekly_usd - cost),
        "percent_used": percent, "period_start": payload["period_start"],
        "period_end": payload["period_end"], "week_starts_on": payload["week_starts_on"],
        "as_of": payload["as_of"], "accounting_status": "settled",
        "accepting_chats": cost < weekly_usd, "status": "ok" if cost < weekly_usd else "exceeded",
        "severity": "red" if percent >= 90 else "orange" if percent >= 80 else "yellow" if percent >= 70 else "normal",
    }
