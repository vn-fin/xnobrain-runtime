...
describe('TeamsView', () => {
  ...
  it('builds a connected DAG by clicking agent cards and node ports', async () => {
    ...
    const viewport = screen.getByTitle('Scroll to zoom · drag the background to pan');
    fireEvent.wheel(viewport, { deltaY: -100 });
    expect(screen.getByRole('button', { name: 'Fit workflow to view' })).toHaveTextContent('116%');
    fireEvent.click(screen.getByRole('button', { name: 'Zoom out workflow' }));
    expect(screen.getByRole('button', { name: 'Fit workflow to view' })).toHaveTextContent('106%');
  ...
  it('highlights active graph nodes, reveals returned output, and cancels the run', async () => {
    ...
    const run: TeamRunRecord = {
      ...
      coordinator_conversation_id: 'coordinator-session',
      synthesis_conversation_id: 'synthesis-session',
      created_at: '2026-07-27T04:35:29Z',
      started_at: '2026-07-27T04:35:29Z',
      ended_at: null,
      updated_at: '2026-07-27T04:35:42Z',
    ...
    const onConversationNavigate = vi.fn();
    ...
    expect(screen.getByLabelText('Start agent metrics')).toHaveTextContent('model');
    expect(screen.getByLabelText('Coordinator agent metrics')).toHaveTextContent('model');
    ...
    expect(onConversationNavigate).toHaveBeenCalledWith('researcher', 'conversation-1');
    ...
    expect(onOpenChat).toHaveBeenCalledWith('researcher', 'conversation-1');
    const workedToggle = screen.getByRole('button', { name: /Worked/i });
    expect(workedToggle).toHaveAttribute('aria-expanded', 'false');
    ...
    const terminalSessionButtons = screen.getAllByRole('button', { name: 'View Team Lead session' });
    fireEvent.click(terminalSessionButtons[0]);
    expect(onConversationNavigate).toHaveBeenLastCalledWith('lead', 'coordinator-session');
    fireEvent.click(await screen.findByRole('button', { name: 'Close node session' }));
    fireEvent.click(terminalSessionButtons[1]);
    expect(onConversationNavigate).toHaveBeenLastCalledWith('lead', 'synthesis-session');
    fireEvent.click(await screen.findByRole('button', { name: 'Close node session' }));
