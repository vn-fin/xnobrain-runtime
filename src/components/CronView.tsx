...
export function CronView({
  ...
  routeJobId?: string;
  routeAgentId?: string;
  onNavigate?: (jobId: string, agentId?: string) => void;
}) {
  ...
  useEffect(() => {
    ...
    if (!routeJobId) {
      if (!selectedId && !selectedAgentId) return;
      setSelectedId('');
      setSelectedAgentId('');
      setSelectedRunId('');
    ...
    const agentIdForRoute = routeAgentId || crons.find((job) => job.id === routeJobId)?.agentId || '';
    if (!agentIdForRoute || (selectedId === routeJobId && selectedAgentId === agentIdForRoute)) return;
    setSelectedId(routeJobId);
    setSelectedAgentId(agentIdForRoute);
    setSelectedRunId('');
    void onLoadDetail(routeJobId, agentIdForRoute);
  }, [
    crons, onCloseDetail, onLoadDetail, routeAgentId, routeControlled, routeJobId,
