...
const TASK_SKILL_PREVIEW_COUNT = 8;
    ...
    <div className="kb-overlay" onClick={onClose}>
              ...
              <>
                ...
                {task.skills.length > 0 && (
                  <div className="kb-task-skills">
                    {(skillsExpanded ? task.skills : task.skills.slice(0, TASK_SKILL_PREVIEW_COUNT))
                      .map((skill) => <span key={skill}>{skill}</span>)}
                    {task.skills.length > TASK_SKILL_PREVIEW_COUNT && (
                      <button
                        type="button"
                        className="kb-task-skills-toggle"
                        aria-expanded={skillsExpanded}
                        aria-label={skillsExpanded
                          ? 'Show fewer skills'
                          : `Show ${task.skills.length - TASK_SKILL_PREVIEW_COUNT} more skills`}
                        onClick={() => setSkillsExpanded((current) => !current)}
                      >
                        {skillsExpanded ? 'Show fewer' : `+${task.skills.length - TASK_SKILL_PREVIEW_COUNT} more`}
                        <ChevronDown size={13} />
                      </button>
                    )}
                  </div>
            ...
            <section className="kb-detail-section kb-team-run">
              <div className="kb-section-heading">
                <div>
                  ...
                  <small>Live task state for every DAG node.</small>
                </div>
            ...
            <section className="kb-detail-section kb-conversation-tracking">
              <div className="kb-section-heading">
                <div>
                  ...
                  <small>Follow the worker’s stored agent session.</small>
                </div>
      ...
      {conversationOpen && conversation && (
        <SessionConversationModal
          ...
          onOpenInConversation={onOpenChat
            ? () => onOpenChat(conversation.agentId, conversation.id)
            : undefined}
