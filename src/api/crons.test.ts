...
describe('cronsApi', () => {
  ...
  it('creates through the XNOBrain cron wrapper', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('/xnobrain/api/runtime/v1/cron/jobs');
  ...
  it('maps wrapper job detail and latest run', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('/xnobrain/api/runtime/v1/cron/jobs/job-1');
  });
  ...
  it('maps the pending run returned by Run now', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('/xnobrain/api/runtime/v1/cron/jobs/job-1/run');
  });
