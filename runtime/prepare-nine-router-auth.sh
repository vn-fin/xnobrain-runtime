#!/usr/bin/env bash
# Prepare the private 9router CLI identity shared with XNOBrain Studio.
set -euo pipefail
...
umask 077
printf '%s' 'xnobrain-runtime' >"$data_directory/machine-id"
printf '%s' "${NINE_ROUTER_INTERNAL_SECRET:-xnobrain-local-9router}" >"$data_directory/auth/cli-secret"
