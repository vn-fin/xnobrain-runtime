...
1. `POST /xnobrain/api/runtime/v1/bundles/exports` prepares a file-backed export and returns an
   export ID, filename, SHA-256, byte size, chunk size, and part count.
2. `GET /xnobrain/api/runtime/v1/bundles/exports/{id}/parts/{part}` downloads one part.
3. `DELETE /xnobrain/api/runtime/v1/bundles/exports/{id}` removes the transfer.
4. `POST /xnobrain/api/runtime/v1/bundles/uploads` starts an upload with filename and byte size.
5. `PUT /xnobrain/api/runtime/v1/bundles/uploads/{id}/parts/{part}` uploads one exact-size
   binary part. Parts are idempotently replaceable and may arrive out of order.
6. `POST /xnobrain/api/runtime/v1/bundles/uploads/{id}/complete` verifies and merges all parts,
...
7. `POST /xnobrain/api/runtime/v1/bundles/uploads/{id}/apply` atomically publishes the import;
