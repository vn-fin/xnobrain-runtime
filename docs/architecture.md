...
Open Lumora is a Python modular monolith layered onto the original Hermes CLI
...
```text
Traefik -> React
        -> FastAPI (Hermes native routes + Open Lumora routes)
...
Open Lumora route assembly is centralized in `open_lumora/routes/setup.py`.
...
skills, tools, provider, webhook, and gateway APIs. Open Lumora adds stable UI
...
profile file, not an Open Lumora database or schema.
