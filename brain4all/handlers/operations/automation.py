...
def operations(handler: Any, request: Any, body: dict[str, Any]) -> dict[str, Operation]:
    ...
    return {
        ...
        "cron_blueprints": (s.list_cron_blueprints, "cron blueprints retrieved", 200),
        "cron_blueprint_instantiate": (lambda: s.instantiate_cron_blueprint(body), "cron job created from blueprint", 201),
        "cron_delivery_targets": (lambda: s.list_cron_delivery_targets(q.get("agent_id")), "cron delivery targets retrieved", 200),
        "cron_job_targets_list": (lambda: s.list_cron_job_targets(p["job_id"]), "cron job delivery targets retrieved", 200),
        "cron_job_target_add": (lambda: s.add_cron_job_target(p["job_id"], body), "cron delivery target added", 201),
        "cron_job_target_remove": (lambda: s.remove_cron_job_target(p["job_id"], p["target_id"]), "cron delivery target removed", 200),
        "cron_trigger": (lambda: s.run_cron(p["job_id"]), "cron job triggered", 200),
        "cron_runs": (lambda: s.list_cron_runs(p["job_id"], int(q.get("limit") or 20)), "cron runs retrieved", 200),
