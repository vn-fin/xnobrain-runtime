...
class APIHandlers:
    ...
    def _operation(self, name: str, request: Request, body: dict[str, Any]) -> tuple[Callable[[], Any], str, int]:
        ...
        operations: dict[str, tuple[Callable[[], Any], str, int]] = {
            ...
            "cron_list": (s.list_crons, "cron jobs retrieved successfully", 200), "cron_get": (lambda: s.get_job_detail(p["job_id"]), "cron job detail retrieved successfully", 200), "cron_create": (lambda: s.create_cron(body), "cron job created successfully", 201),
            ...
            "cron_run": (lambda: s.run_cron(p["job_id"]), "cron job triggered", 200), "cron_delete": (lambda: s.delete_cron(p["job_id"]), "cron job deleted", 200),
            ...
            "analytics_overview": (lambda: s.analytics.usage_overview(agent_ids=_csv(q.get("agents")), **_range(q), bucket=_bucket(q)), "usage overview retrieved successfully", 200),
            ...
            "analytics_models": (lambda: s.analytics.models_breakdown(agent_ids=_csv(q.get("agents")), **_range(q), bucket=_bucket(q)), "model usage retrieved successfully", 200),
    ...
    async def workspace_upload_chunk(self, request: Request) -> Response:
        try:
            ...
            if len(content) > WORKSPACE_UPLOAD_CHUNK_BYTES:
                error = ValueError(
                    f"chunk is too large (max {WORKSPACE_UPLOAD_CHUNK_BYTES} bytes)"
                )
