...
class CronDeliveryAPITests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_blueprints_targets_and_runs_routes(self):
        async with AsyncClient(transport=ASGITransport(app=self.app), base_url="http://test") as client:
            ...
            blueprints = await client.get("/api/brain/v1/cron/blueprints")
            ...
            self.composition.service.cron.reconcile_deliveries()
            options = await client.get(f"/api/brain/v1/cron/delivery-targets?agent_id={agent_id}")
            self.assertEqual(options.status_code, 200, options.text)
            self.assertTrue(any(item["target_type"] == "kanban" for item in options.json()["data"]["options"]))
            runs = await client.get(f"/api/brain/v1/cron/jobs/{job_id}/runs")
            ...
            invalid_limit = await client.get(f"/api/brain/v1/cron/jobs/{job_id}/runs?limit=nope")
            self.assertEqual(invalid_limit.status_code, 400, invalid_limit.text)

            traversal = await client.post(f"/api/brain/v1/cron/jobs/{job_id}/delivery-targets", json={
                "target_type": "file", "destination": "../outside.md",
            })
            self.assertIn(traversal.status_code, {400, 422}, traversal.text)

            email = await client.post(f"/api/brain/v1/cron/jobs/{job_id}/delivery-targets", json={
                "target_type": "email", "destination": "owner@example.test",
            })
            self.assertEqual(email.status_code, 201, email.text)
            detail = await client.get(f"/api/brain/v1/cron/jobs/{job_id}")
            self.assertEqual(detail.status_code, 200, detail.text)
            self.assertIn("email:owner@example.test", detail.json()["data"]["job"]["deliver"])

            kanban = await client.post(f"/api/brain/v1/cron/jobs/{job_id}/delivery-targets", json={
            ...
            invalid = await client.post(f"/api/brain/v1/cron/jobs/{job_id}/delivery-targets", json={
    ...
    async def test_run_now_dispatches_the_selected_agent_profile(self):
        with patch.object(self.composition.service.cron, "fire_due", return_value=True) as fire_due:
            async with self.app.router.lifespan_context(self.app):
                async with AsyncClient(transport=ASGITransport(app=self.app), base_url="http://test") as client:
                    created = await client.post("/api/brain/v1/agents", json={"name": "Profile Cron"})
                    agent_id = created.json()["data"]["id"]
                    cron = await client.post("/api/brain/v1/cron/jobs", json={
                        "agent_id": agent_id,
                    ...
                    job_id = cron.json()["data"]["id"]
                    triggered = await client.post(f"/api/brain/v1/cron/jobs/{job_id}/run")
    ...
    async def test_instantiates_real_blueprint_defaults(self):
        async with AsyncClient(transport=ASGITransport(app=self.app), base_url="http://test") as client:
            created = await client.post("/api/brain/v1/agents", json={"name": "Blueprint Owner"})
            agent_id = created.json()["data"]["id"]
            response = await client.post("/api/brain/v1/cron/blueprints/instantiate", json={
