#!/usr/bin/env python3
"""Scaffold a new Hermes plugin (tool + hook + slash command) from the template.
...
Default dest is ``./.tools/hermes-agent/plugins`` (the vendored source). For a
user-installable plugin, pass ``--dest ~/.hermes/plugins`` or copy the folder
there afterwards, then ``hermes plugins enable <name>``.
...
REPO_ROOT = Path.cwd()
DEFAULT_DEST = REPO_ROOT / ".tools" / "hermes-agent" / "plugins"
...
def main() -> int:
    ...
    ap.add_argument("--dest", type=Path, default=DEFAULT_DEST,
                    help="parent dir to create the plugin folder in")
    ...
    print(f"        hermes plugins list   # confirm discovery")
    return 0
